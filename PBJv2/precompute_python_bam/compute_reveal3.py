...
if __name__ == '__main__':
    ...
    off_rank = list(itertools.combinations(_RANKS, 3)) #all combinations that don't have same rank (e.g. [(A, K, Q), (A, Q), (A, J)...])
    # same_rank = list(zip(_RANKS, _RANKS, _RANKS)) #all same ranks [(A, A, A), (K, K, K), (Q, Q, Q)...]
    same_rank = list(zip(pocket_pair_holds, _RANKS)) # [(A, A, A), (A, A, K), (A, A, Q)]

    suited_strengths = [calculate_strength([card[0] + 'c', card[1] + 'c', card[2] + 'h' ], _MONTE_CARLO_ITERS) for card in off_rank] #all holes with the same suit
    two_same_suit_strengths = [calculate_strength([card[0] + 'c', card[1] + 'c', card[2] + 'd'], _MONTE_CARLO_ITERS) for card in off_rank] #all holes with off suits
    off_suit_strengths = [calculate_strength([card[0] + 'c', card[1] + 'd', card[2] + 'h'], _MONTE_CARLO_ITERS) for card in off_rank] #all holes with off suits
    pocket_pair_strengths = [calculate_strength([card[0] + 'c', card[1] + 'd'], _MONTE_CARLO_ITERS) for card in same_rank] #pocket pairs must be off suit

    suited_holes = [hole[0] + hole[1] + 's' for hole in off_rank] #s == suited
    off_suited_holes = [hole[0] + hole[1] + 'o' for hole in off_rank] #o == off-suit
    pocket_pairs = [hole[0] + hole[1] + 'o' for hole in same_rank] #pocket pairs are always off suit
    ...
    hole_df.to_csv('strength_preflop.csv', index=False) #save it for later use, trade space for time!